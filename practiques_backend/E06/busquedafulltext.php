...
<body>
    <h1>Resultats de Búsqueda (Full Text)</h1>
    <?php
        require_once("conexion_pdo.php");
        $db = new Conexion();

        $queryString = $_GET['q'];
        $result = $db -> prepare("SELECT id,titol FROM e06noticies WHERE visible=1 AND MATCH (titol,text) AGAINST (:qString IN BOOLEAN MODE)");
