...
// JSON PARSE
fetch('https://citmalumnes.upc.es/~paugv2/js/items.json')
//fetch('/js/items.json')
    .then(response => response.json())
    .then(data => {
        data.items.forEach(item => {
            if(item.mobileReady == isMobileBrowser || item.desktopReady == !isMobileBrowser) {
                createItem(item);
            }
        });
    });
...
function createItem(item) {
    ...
    masterDiv.className = 'item';
    container.appendChild(masterDiv);

        let title = document.createElement('div');
        title.className = 'title';
        title.setAttribute('onclick','expand(event,this)');
        masterDiv.appendChild(title);
            ...
            title.appendChild(titleSpan);
                ...
                titleIcon.setAttribute('name',item.icon);
                ...
                if(item.strong){
                    ...
                    titleText = document.createTextNode(' ' + item.title);
                }
                titleSpan.appendChild(titleText);

            let arrowSpan = document.createElement('span');
            arrowSpan.className = 'more-arrow';
            title.appendChild(arrowSpan);

        let desc = document.createElement('div');
        desc.className = 'desc';
        masterDiv.appendChild(desc);
        
            let descText = document.createElement('div');
            descText.className = 'desc-text';
            descText.innerHTML = item.desc;
            desc.appendChild(descText);
            
            let descBtnDiv = document.createElement('div');
            descBtnDiv.className = 'btn-wrapper';
            descBtnDiv.innerHTML = (item.desktopReady ? '<ion-icon name="laptop-outline"></ion-icon>' : '') + (item.mobileReady ? '<ion-icon name="phone-portrait-outline"></ion-icon>' : '');
            desc.appendChild(descBtnDiv);

                

                let descBtn = document.createElement('a');
                descBtn.href = item.href;
                descBtn.target = item.target;
                descBtn.innerHTML = item.btnText;
                descBtnDiv.appendChild(descBtn);
}

function expand(ev,el){
    ev.preventDefault();

    thisDesc = el.parentNode.querySelector('.desc');
    thisArrow = el.querySelector('.more-arrow');
    ...
    document.querySelectorAll('.more-arrow').forEach(e => {
        if(e != thisArrow){
